...
# creating instances
time for i in $( seq 0 $nodes ); do nova boot --image $infra_cirros_id --flavor infra_flavor --availability-zone nova:${cloud}-compute-std-$i.localdomain --nic net-id=${net} --config-drive true --security-groups $infra_secgroup_id prov1-vm-$i; done
