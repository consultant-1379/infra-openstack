resource "libvirt_volume" "ieatrheldirdeva_vol"{
...
resource "libvirt_cloudinit_disk" "ieatrheldirdeva_cloudinit_disk" {
...
meta_data = <<EOF
instance-id: "ieatrheldirdeva-001"
local-hostname: "ieatrheldirdeva"
EOF
}
resource "libvirt_domain" "ieatrheldirdeva" {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldirdeva_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldirdeva_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
