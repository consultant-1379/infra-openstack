resource "libvirt_volume" "ieatrheldirdevb_vol"{
  name = "ieatrheldirdevb_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldirdevb_cloudinit_disk" {
  name = "ieatrheldirdevb_cloudinit_disk"
...
local-hostname: "ieatrheldirdevb"
disable_root: 0
ssh_pwauth: 1
...
meta_data = <<EOF
instance-id: "ieatrheldirdevb-001"
local-hostname: "ieatrheldirdevb"
EOF
}
resource "libvirt_domain" "ieatrheldirdevb" {
  name = "ieatrheldirdevb"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldirdevb_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldirdevb_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
