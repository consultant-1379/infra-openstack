resource "libvirt_volume" "ieatrheldir17b_vol"{
  name = "ieatrheldir17b_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir17b_cloudinit_disk" {
  name = "ieatrheldir17b_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir17b-001"
local-hostname: "ieatrheldir17b"
EOF
}
resource "libvirt_domain" "ieatrheldir17b" {
  name = "ieatrheldir17b"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir17b_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir17b_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
