resource "libvirt_volume" "ieatrheldir18c_vol"{
  name = "ieatrheldir18c_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir18c_cloudinit_disk" {
  name = "ieatrheldir18c_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir18c-001"
local-hostname: "ieatrheldir18c"
EOF
}
resource "libvirt_domain" "ieatrheldir18c" {
  name = "ieatrheldir18c"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir18c_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir18c_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
