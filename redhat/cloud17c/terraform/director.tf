resource "libvirt_volume" "ieatrheldir17c_vol"{
  name = "ieatrheldir17c_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir17c_cloudinit_disk" {
  name = "ieatrheldir17c_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir17c-001"
local-hostname: "ieatrheldir17c"
EOF
}
resource "libvirt_domain" "ieatrheldir17c" {
  name = "ieatrheldir17c"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir17c_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir17c_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
