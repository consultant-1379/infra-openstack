resource "libvirt_volume" "ieatrheldir17a_vol"{
  name = "ieatrheldir17a_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir17a_cloudinit_disk" {
  name = "ieatrheldir17a_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir17a-001"
local-hostname: "ieatrheldir17a"
EOF
}
resource "libvirt_domain" "ieatrheldir17a" {
  name = "ieatrheldir17a"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir17a_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir17a_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
