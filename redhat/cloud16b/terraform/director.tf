resource "libvirt_volume" "ieatrheldir16B_vol"{
  name = "ieatrheldir16B_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir16B_cloudinit_disk" {
  name = "ieatrheldir16B_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir16B-001"
local-hostname: "ieatrheldir16B"
EOF
}
resource "libvirt_domain" "ieatrheldir16B" {
  name = "ieatrheldir16B"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir16B_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir16B_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
