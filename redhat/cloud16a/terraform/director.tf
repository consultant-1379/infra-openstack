resource "libvirt_volume" "ieatrheldir16a_vol"{
  name = "ieatrheldir16a_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir16a_cloudinit_disk" {
  name = "ieatrheldir16a_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir16a-001"
local-hostname: "ieatrheldir16a"
EOF
}
resource "libvirt_domain" "ieatrheldir16a" {
  name = "ieatrheldir16a"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir16a_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir16a_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
