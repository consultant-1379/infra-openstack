resource "libvirt_volume" "ieatrheldir6a_vol"{
  name = "ieatrheldir6a_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir6a_cloudinit_disk" {
  name = "ieatrheldir6a_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir6a-001"
local-hostname: "ieatrheldir6a"
EOF
}
resource "libvirt_domain" "ieatrheldir6a" {
  name = "ieatrheldir6a"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir6a_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir6a_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
