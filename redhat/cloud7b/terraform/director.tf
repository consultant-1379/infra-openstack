resource "libvirt_volume" "ieatrheldir7b_vol"{
  name = "ieatrheldir7b_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir7b_cloudinit_disk" {
  name = "ieatrheldir7b_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir7b-001"
local-hostname: "ieatrheldir7b"
EOF
}
resource "libvirt_domain" "ieatrheldir7b" {
  name = "ieatrheldir7b"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir7b_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir7b_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
