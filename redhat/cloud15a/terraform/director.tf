resource "libvirt_volume" "ieatrheldir15a_vol"{
  name = "ieatrheldir15a_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldir15a_cloudinit_disk" {
  name = "ieatrheldir15a_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "ieatrheldir15a-001"
local-hostname: "ieatrheldir15a"
EOF
}
resource "libvirt_domain" "ieatrheldir15a" {
  name = "ieatrheldir15a"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldir15a_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldir15a_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
