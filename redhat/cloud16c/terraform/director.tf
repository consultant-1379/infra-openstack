resource "libvirt_volume" "NXDOMAI_vol"{
  name = "NXDOMAI_vol"
...
resource "libvirt_cloudinit_disk" "NXDOMAI_cloudinit_disk" {
  name = "NXDOMAI_cloudinit_disk"
...
meta_data = <<EOF
instance-id: "NXDOMAI-001"
local-hostname: "NXDOMAI"
EOF
}
resource "libvirt_domain" "NXDOMAI" {
  name = "NXDOMAI"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.NXDOMAI_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.NXDOMAI_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
