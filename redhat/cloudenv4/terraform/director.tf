resource "libvirt_volume" "ieatrheldirenv4_vol"{
  name = "ieatrheldirenv4_vol"
...
resource "libvirt_cloudinit_disk" "ieatrheldirenv4_cloudinit_disk" {
  name = "ieatrheldirenv4_cloudinit_disk"
...
local-hostname: "ieatrheldirenv4"
disable_root: 0
ssh_pwauth: 1
...
meta_data = <<EOF
instance-id: "ieatrheldirenv4-001"
local-hostname: "ieatrheldirenv4"
EOF
}
resource "libvirt_domain" "ieatrheldirenv4" {
  name = "ieatrheldirenv4"
  memory = "32768"
  vcpu = 16
  network_interface {
  ...
  cloudinit = libvirt_cloudinit_disk.ieatrheldirenv4_cloudinit_disk.id
  disk {
   volume_id = libvirt_volume.ieatrheldirenv4_vol.id
   scsi = "true"
  }
  video {
    type = "vga"
